  ...
  (with-fixture jvm-context (*kotlin-path* 'ast-index-memory)
    (is (equal
          ...
          (get-values-from-request-mapping
            :kotlin
            (first (trav:get-asts
                     (ast '((:path . "p1/server/spring/src/main/p1/RequestMappingDefinition.kt")
                            (:line . 8) (:offset . 1)))
                     '("ANNOTATION_ENTRY"))))))))
  ...
  (with-fixture jvm-context (*kotlin-path* 'ast-index-memory)
    (is (equal
          ...
          (get-values-from-request-mapping
            :kotlin
            (first (trav:get-asts
                     (ast '((:path . "p1/server/spring/src/main/p1/GetMappingDefinition.kt")
                            (:line . 13) (:offset . 5)))
                     '("ANNOTATION_ENTRY"))))))))
    ...
    (is (equal
          ...
          (trav:ast-value
            (find-param-from-path-variable
              :kotlin
              (ast '((:path . "p1/server/spring/src/main/p1/PathVariableDefinition.kt")
                     (:line . 11) (:offset . 23)))
              "v")
            "name")))))
