...
(defmethod find-affected-pos ((parser parser-typescript) file-path ast line-no)
  ...
  (let ((q (make-queue))
    ...
    (loop
      (let ((ast (dequeue q)))
        (if (null ast) (return))
        ;;(format t "ast-pos: ~a, ast: ~a~%" ast-pos (inga/utils::top ast 15))
        (when (jsown:keyp ast "kindName")
          (format t "kindName: ~a~%"(jsown:val ast "kindName")))
        ...
        (when (and
          ...
          (let ((init (jsown:val ast "initializer")))
            (format t "kindName: ~a~%" (jsown:val init "kindName"))
